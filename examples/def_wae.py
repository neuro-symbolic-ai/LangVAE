...
from pythae.models.wae_mmd import WAE_MMD_Config
...
def main(config: dict):
    ...
    if (MODE == "train_chkp"):
        ...
        model = LangWAE.load_from_folder(checkpoint_dir)
        ...
        model = LangWAE(model_config, encoder, decoder)

    # model.debug = True

    exp_label = f"{ds_prefix}-langwae-{config['encoder'].replace('/', '__')}-{config['decoder'].replace('/', '__')}-l{latent_size}-m{config['mem_factor']}"
